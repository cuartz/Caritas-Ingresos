...
public class datosChequeAC extends DispatchAction{
    private final static String SUCCESS = "success"; 
    private final static String FAILURE = "failure:";
    
    public ActionForward agregarCheque(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) {
        JSONObject data = new JSONObject();
        JSONObject fInfo = new JSONObject();         
        datosDepositoBL deposito = new datosDepositoBL();
        datosChequeBL cheque    = new datosChequeBL();
        
        try {                       
            String jsonData = request.getParameter("jsonData");
            JSONObject jsonObject = JSONObject.fromObject(jsonData);              
            cheque.agregarCheque(jsonObject, request.getParameter("uid")); 
            int salida = 1;
