...
public class confirmacionPagoAC extends DispatchAction {    
     ...
     public ActionForward getInfoDireccion(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) {
        String salida = "";
        int idBitacora = Integer.parseInt(request.getParameter("ID_BITACORA"));
        confirmacionPagoBL confirmPagoBL = new confirmacionPagoBL();
        
        try {
            salida = confirmPagoBL.getInfoDireccion(idBitacora);            
            request.setAttribute("resp", salida);
        } catch (Exception e) {
        }
        return mapping.findForward(SUCCESS);
